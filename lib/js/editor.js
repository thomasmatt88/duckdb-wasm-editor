...
const iframeName = window.name; // Only use localStorage if window.name is explicitly set -> will be set in parent iframe as name=""
const STORAGE_KEY = iframeName;
const hash = new URLSearchParams(location.hash.slice(1));
let defaultCode = hash.get("defaultCode");

// Check localStorage first for persisted code (only if iframeName is set)
let storedCode = null;
if (STORAGE_KEY) {
...
window.addEventListener("message", (event) => {
  ...
  if (typeof event.data === "string") {
    ...
    // Persist to localStorage only if iframeName is set
    if (STORAGE_KEY) {

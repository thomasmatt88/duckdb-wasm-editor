...
async function runQueries() {
  console.log(document);
  let queries = codeEditor.getValue();
  ...
  try {
    ...
    // only one query at a time -> https://github.com/duckdb/duckdb-wasm/discussions/1963
    // This regex splits by semicolons that are NOT inside single quotes
    const queryArray = queries
      .split(/;(?=(?:[^']*'[^']*')*[^']*$)/)
      .map((q) => q.trim())
      .filter((q) => q.length > 0);
    for (const statement of queryArray) {
      try {
        const result = await conn.query(statement);
        console.log("Success:", statement.slice(0, 30) + "...");
        arrowToHtmlTable(result, "resultTable");
      } catch (err) {
        console.error("Query failed:", statement);
        console.error("Error details:", err);
        // Determine if you want to stop the loop or continue to the next
        throw err;
      }
    }
